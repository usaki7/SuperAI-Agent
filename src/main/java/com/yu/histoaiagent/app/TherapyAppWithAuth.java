 ...
 * 
 * Advisor执行顺序：
 * 1. AuthorizationAdvisor (order=0) - 权限校验
 * 2. MessageChatMemoryAdvisor (order=100) - 加载历史消息
 * 3. MyLoggerAdvisor (order=200) - 日志记录
 */
@Component
@Slf4j
public class TherapyAppWithAuth {
     ...
     * 构造函数 - 集成权限校验
     */
    public TherapyAppWithAuth(
            ChatModel dashscopeChatModel,
            RedisChatMemory redisChatMemory,
            AuthorizationAdvisor authorizationAdvisor) {
        
        log.info("Initializing TherapyAppWithAuth with Authorization");
        ...
        log.info("TherapyAppWithAuth initialized with Advisors: Authorization -> ChatMemory -> Logger");
    }
     ...
     * @param conversationId 会话ID（必须）
     * @return AI回复
     */
    public String doChat(String message, String userId, String conversationId) {
        log.info("Processing chat with auth - userId: {}, conversationId: {}, message: {}", 
     ...
     * 管理员模式对话 - 跳过权限检查
     */
    public String doChatAsAdmin(String message, String conversationId) {
        log.info("Admin chat - conversationId: {}", conversationId);
        ...
        ChatResponse chatResponse = chatClient
                ...
                .advisors(advisorSpec -> {
                    // ✅ 跳过权限检查
                    advisorSpec.param(AuthorizationAdvisor.SKIP_AUTH_CHECK, true);
                    advisorSpec.param(ChatMemory.CONVERSATION_ID, conversationId);
                })
                .call()
                .chatResponse();
     ...
     * AI对话结构化输出报告 - 带权限校验
     */
    public TherapyReport doChatWithReport(String message, String userId, String conversationId) {
        log.info("Generating therapy report with auth - userId: {}, conversationId: {}", 
